...
μία τιμή.
Μία καλύτερη;;; λύση στο προηγούμενο πρόβλημα. 
=====================================================================
*/
...
int main (int argc, char **argv)
{
    ...
    for (r = 0; r < 1000; r++) 
        Matrix [r] = 0;
    M = (pthread_mutex_t *) malloc (1000 * sizeof (pthread_mutex_t));
    for (r = 0; r < 1000; r++)
        pthread_mutex_init (&M[r], NULL);
    Dat1.ID = 1;
    Dat1.Mat = Matrix;
    Dat1.Size = 1000;
    Dat1.Mutex = M;
...
void *tfunc1 (void *p)
{
    ...
    for (i = 0; i <= t->Size; i++)
    {
        pthread_mutex_lock (&t->Mutex[i]);
        int v = t->Mat[i];
        usleep (rand () % 10000);
        v = v + t->ID;
        t->Mat[i] = v;
        pthread_mutex_unlock (&t->Mutex[i]);
    }
}
